...
void MCTS::expand(MCTSNode* node, const std::unordered_set<uint64_t>& history){
    ...
    auto passChild = std::make_unique<MCTSNode>(
        node->state.makePass(), -1, node);
    node->children.push_back(std::move(passChild));
}
...
double MCTS::simulate(boardstate state){
    ...
    while(state.passCount < 2 && maxMoves-- > 0){
        ...
        for(int i = 0; i < 6; ++i){
            ...
            while(bits){
                ...
                candidates.push_back(i * 64 + bit);
                bits &= bits - 1;
            }
        }
        // shuffle and try until we find a legal move
    ...
    return state.score(6.5f) > 0 ? 1.0 : 0.0;
